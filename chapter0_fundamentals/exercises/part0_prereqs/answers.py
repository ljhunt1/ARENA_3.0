...
# %%
# column stacking
arr1 = einops.rearrange(arr, 'n c h w -> c (n h) w')
...
arr3 = einops.repeat(arr[0:2], 'n c h w -> c (n h) (2 w)')
...
def rearrange_1() -> Tensor:
    flat = t.arange(3, 9)
    return einops.rearrange(flat, '(b1 b2) -> b1 b2', b1 = 3)
...
def temperatures_average(temps: Tensor) -> Tensor:
    ...
    return einops.reduce(temps, '(wk day) -> wk', 'mean', day=7)
...
def temperatures_differences(temps: Tensor) -> Tensor:
    ...
    """
    assert len(temps) % 7 == 0
    wk_averages = einops.reduce(temps, '(wk 7) -> wk', 'mean')
    return temps - einops.repeat(wk_averages, 'wk -> (wk 7)')
...
def temperatures_normalized(temps: Tensor) -> Tensor:
    ...
    """
    wk_averages = einops.repeat(einops.reduce(temps, '(wk 7) -> wk', 'mean'), 'wk -> (wk 7)')
    wk_std = einops.repeat(einops.reduce(temps, '(wk 7) -> wk', t.std), 'wk -> (wk 7)')
    return (temps - wk_averages) / wk_std
...
def cos_sim_matrix(matrix: Tensor) -> Tensor:
    ...
    matrix: shape (m, n)
    """
    normed = matrix / t.norm(matrix, dim=1, keepdim=True)
    # return normed @ normed.T
    return t.einsum('ik,jk->ij', normed, normed)
...
def sample_distribution(probs: Tensor, n: int) -> Tensor:
    ...
    rands = t.rand(size=(n,))
    return t.sum(cdf.unsqueeze(0) < rands.unsqueeze(-1), dim=1)
...
def classifier_accuracy(scores: Tensor, true_classes: Tensor) -> Tensor:
    ...
    Use t.argmax.
    """
    return t.sum((scores.argmax(dim=1) == true_classes)) / len(scores)
    raise NotImplementedError()
...
def gather_2d(matrix: Tensor, indexes: Tensor) -> Tensor:
    ...
    assert(t.all(indexes < matrix.shape[1]))
...
def batched_logsumexp(matrix: Tensor) -> Tensor:
    ...
    """
    row_max = einops.reduce(matrix, 'i j -> i 1', 'max')
    logsumexp = row_max + t.log(t.sum(t.exp(matrix - row_max), dim=1, keepdim=True))
    return logsumexp.flatten()
...
def batched_softmax(matrix: Tensor) -> Tensor:
    ...
    """
    capped = matrix - einops.reduce(matrix, 'i j -> i 1', 'max')
    return t.exp(capped) / einops.reduce(t.exp(capped), 'i j -> i 1', 'sum')
...
def batched_logsoftmax(matrix: Tensor) -> Tensor:
    ...
    """
    capped = matrix - einops.reduce(matrix, 'i j -> i 1', 'max')
    return t.log(t.exp(capped) / t.sum(t.exp(capped), dim=1, keepdim=True))
...
def batched_cross_entropy_loss(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    row_max = einops.reduce(logits, 'i j -> i 1', 'max')
    capped = logits - row_max
    probs = t.exp(capped) / t.sum(t.exp(capped), dim=1, keepdim=True)

    # cross entropy on a single classifier is -log(prob) on the expected output
    return -t.log(probs[np.arange(len(true_labels)),true_labels])
